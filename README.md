...
- **Push to `main`** → `check` → `build` (arm64 `prod` image → `ghcr.io/maciej-jedral/pinch-backend:sha-<commit>` + `latest`) → `deploy` (SSH to the EC2 box: copy `compose.prod.yml` + a generated `.env`, `pull`, run migrations, `up -d`, smoke-test `/api/hello`).
